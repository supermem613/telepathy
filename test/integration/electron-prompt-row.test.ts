...
async function activeTerminalBottomState(hostSelector: string): Promise<TerminalBottomState> {
  assert.ok(page, "Electron page should have loaded");
  return page.evaluate((selector) => {
    const rows = Array.from(document.querySelectorAll(`${selector} .xterm-rows > div`));
...
async function screenshotTextBottomGap(hostSelector: string): Promise<{ bottomGap: number; hostHeight: number }> {
  assert.ok(page, "Electron page should have loaded");
  const png = await page.screenshot({ type: "png" });
  const dataUrl = `data:image/png;base64,${png.toString("base64")}`;
  return page.evaluate(async ({ selector, dataUrl: url }) => {
    const host = document.querySelector(selector);
    if (!host) {
    ...
    return {
      bottomGap: ((bottom - top) - bottomMostInk - 1) / scaleY,
      hostHeight: rect.height,
    };
  }, { selector: hostSelector, dataUrl });
}

async function assertPromptTypingAtVisualBottom(hostSelector: string): Promise<void> {
  await page!.click(hostSelector);
  await page!.keyboard.type("abc");

  await waitForAsync(async () => {
    const text = await page!.evaluate(() => document.body.innerText);
    return text.includes("abc");
  }, { timeout: 5_000, what: "typed input echoed in xterm" });

  const state = await activeTerminalBottomState(hostSelector);
  ...
  assert.ok(state.bottomGap !== null && state.rowHeight !== null && state.bottomGap < state.rowHeight / 2,
    `typed row should sit at the visual bottom of the terminal: ${JSON.stringify(state)}`);

  const pixelState = await screenshotTextBottomGap(hostSelector);
  assert.ok(state.rowHeight !== null && pixelState.bottomGap < state.rowHeight / 2,
    `screenshot ink should reach terminal bottom: ${JSON.stringify({ pixelState, rowHeight: state.rowHeight })}`);
}
...
describe("electron e2e: late app connect preserves bottom prompt row", () => {
  it("typing in the app lands on the same bottom row as the host prompt", async (t) => {
    ...
    }, { timeout: 10_000, what: "host prompt in xterm" });

    await assertPromptTypingAtVisualBottom(".term-host.active");
  });
});
